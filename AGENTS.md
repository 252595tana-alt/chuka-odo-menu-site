...
- Keep the golden dragon tall on the center axis. Hold it front-facing through its solo entrance, then rotate it in the same direction, timing, and two-turn scroll phase as the ten-panel vertical spiral; pointer movement, dragging, swiping, keyboard input, and idle time must not rotate either system directly.
