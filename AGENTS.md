...
- Use the exact visible CTA label `お店に行く` for every actionable visit control, including the tenth menu-panel detail action and the separate final Maps CTA. Do not reintroduce `お席のご案内へ` as a button label.
- While the final visit screen is active, let its lacquer CTA breathe on a three-second ease-in-out loop: keep the deep-jade surface, expand to no more than 1.045 scale, and pulse layered amber-gold light around the double frame. Stop both scaling and pulsing for Reduced Motion.
...
- When the centered closing-line BLUR begins, hold forward page progress at the fully centered heading until the final glyph has resolved. Clamp wheel, touch, keyboard, scrollbar, and programmatic forward jumps through the same progress boundary, but let a deliberate backward scroll cancel the hold. Release automatically after the full sequence; skip the hold entirely for Reduced Motion.
